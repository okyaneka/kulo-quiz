...
<script setup lang="ts">
  ...
  const { mutateAsync: handleNextStep, isLoading: stepLoading } = useMutation({
    mutationFn: async () => {
      ...
      if (current == 'q-id-o-index') {
        parsed = () => parseQuiz()
        successCallback = async (data: unknown) => {
          await setQuiz(data as QuizPayload)
          router.push({ name: 'q-id-o-index-questions' })
          active.value = 'q-id-o-index-questions'
        }
        failedCallback = () => {
          ElMessage.error('quiz_is_not_valid')
        }
      } else if (current == 'q-id-o-index-questions') {
        parsed = () => parseQuestions()
        successCallback = async (data: unknown) => {
          await setQuestions(data as Questions[])
          router.push({ name: 'q-id-o-index-config' })
          active.value = 'q-id-o-index-config'
        }
      ...
      } else if (current == 'q-id-o-index-config') {
        parsed = () => parseConfig()
        successCallback = async (data: unknown) => {
...
<template>
  ...
  >
    ...
    >
      <el-tab-pane name="q-id-o-index" label="1. Data"> </el-tab-pane>
      <el-tab-pane name="q-id-o-index-questions" label="2. Questions">
      </el-tab-pane>
      <el-tab-pane name="q-id-o-index-config" label="3. Config"> </el-tab-pane>
    </el-tabs>
  </el-card>
    ...
    >
      <template #extra>
        <el-space fill style="width: 100%">
          <el-button type="primary" @click="goTo('q-id-o-index-questions')"
